...
static const UInt8 kKeychainItemIdentifier[]    = "com.sweep.Keychain\0";
...
- (id)initWithIdentifier: (NSString *)identifier accessGroup:(NSString *) accessGroup;
{
    if (self = [super init])
    {
        ...
        if (! SecItemCopyMatching((CFDictionaryRef)tempQuery, (CFTypeRef *)&outDictionary) == noErr)
        {
			...
			[_keychainData setObject:identifier forKey:(id)kSecAttrGeneric];
			if (accessGroup != nil)
			{
#if TARGET_IPHONE_SIMULATOR
				// Ignore the access group if running on the iPhone simulator.
				...
				// simulator will return -25243 (errSecNoAccessForItem).
#else
				[_keychainData setObject:accessGroup forKey:(id)kSecAttrAccessGroup];
#endif
			}
		}
        else
...
- (void)resetKeychainItem
{
    ...
    {
        NSMutableDictionary *tempDictionary = [self dictionaryToSecItemFormat:_keychainData];
		junk = SecItemDelete((CFDictionaryRef)tempDictionary);
        NSAssert( junk == noErr || junk == errSecItemNotFound, @"Problem deleting current dictionary." );
    }
    
    // Default attributes for keychain item.
    [_keychainData setObject:@"" forKey:(id)kSecAttrAccount];
    [_keychainData setObject:@"" forKey:(id)kSecAttrLabel];
    [_keychainData setObject:@"" forKey:(id)kSecAttrDescription];
    
	// Default data for keychain item.
    [_keychainData setObject:@"" forKey:(id)kSecValueData];
}
...
- (void)writeToKeychain
{
    ...
    else
    {
        ...
        result = SecItemAdd((CFDictionaryRef)[self dictionaryToSecItemFormat:_keychainData], NULL);
